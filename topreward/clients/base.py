...
from topreward.utils.aliases import Event, ImageEvent, ImageNumpy, TextEvent
from topreward.utils.constants import PromptPhraseKey
from topreward.utils.data_types import Episode
from topreward.utils.errors import MaxRetriesExceededError
from topreward.utils.rate_limiter import SECS_PER_MIN, RateLimiter
...
class BaseModelClient(ABC):
    ...
    def generate_response(
        ...
        *,
        ...
        Args:
            ...
            prompt_phrases: Dict of phrase substitutions keyed by PromptPhraseKey values.
        Returns:
    ...
    def _validate_and_normalize_prompt_phrases(self, phrases: dict[str, str]) -> dict[str, str]:
        ...
        if missing:
            raise ValueError(f"Missing or invalid (empty) prompt phrases for required keys: {', '.join(missing)}")
    ...
    def _iter_prompt_events(
        ...
        *,
        ...
        if eval_episode.starting_frame is None:
            raise ValueError("starting_frame must not be None when generating a response")
